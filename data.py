...
def get_dataset(name, split='train', transform=None,
    ...
    train = (split == 'train')
    if name == 'cifar10':
        train_val = datasets.CIFAR10(root=_dataset_path['cifar10'],
                                train=train,
                                transform=transform,
                                target_transform=target_transform,
                                download=download)
        if train == True:
          train_subset, val_subset = torch.utils.data.random_split(train_val, [40000, 10000], generator=torch.Generator().manual_seed(1))
        else:
